...
const MobileWorkersList = () => {
  ...
  const getActions = (worker) => {
    ...
    // 🔧 v22.3: الشهادة الصحية - منطق ذكي مبني على healthCertificateExpiryDate من worker
    const approvedCert = approvedCertificates[worker.id];
    const activeCert = activeCertificates[worker.id];
    const hasValidCert = worker.healthCertificateExpiryDate && new Date(worker.healthCertificateExpiryDate) > new Date();
    ...
    } else if (approvedCert) {
      // ✅ شهادة معتمدة (آخر طلب) → عرض
      actions.push({
        icon: FileHeart, label: 'عرض الشهادة', color: 'green',
        onClick: () => handleViewApprovedCertificate(approvedCert.id),
      });
    } else if (hasValidCert && canRequest) {
      // عنده شهادة سارية لكن لا approvedCert في API → نسمح بطلب جديدة (للتجديد)
      actions.push({
        icon: FileHeart, label: 'طلب تجديد', color: 'green',
        onClick: () => setCertModalWorker(worker),
      });
    } else if (canRequest) {
      // ➕ لا توجد شهادة → طلب جديد
      actions.push({
    ...
    // التقرير الطبي - منطق مماثل
    const approvedReport = approvedReports[worker.id];
    const activeReport = activeReports[worker.id];
    const hasValidReport = worker.medicalReportExpiryDate && new Date(worker.medicalReportExpiryDate) > new Date();
    ...
    if (activeReport) {
      ...
      actions.push({
        icon: Stethoscope, label: hasValidReport ? 'طلب تجديد' : 'طلب تقرير', color: 'yellow',
  ...
  // 🔧 v22.3: استخدام healthCertificateExpiryDate من worker مباشرة (مثل WorkersList الديسكتوب)
  const getStatusBadge = (worker) => {
    const activeCert = activeCertificates[worker.id];
    if (activeCert) {
      return <MobileBadge color="purple">قيد الإصدار</MobileBadge>;
    }
    
    if (!worker.healthCertificateExpiryDate) {
      return <MobileBadge color="gray">لم تُصدر</MobileBadge>;
    }
    
    const expDate = new Date(worker.healthCertificateExpiryDate);
