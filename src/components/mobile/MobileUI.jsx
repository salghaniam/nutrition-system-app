...
export const MobileWorkerCard = ({ worker, onClick, actions = [], badge }) => {
  // 🔧 v22.3: استخدام الحقول الصحيحة من DB
  const name = worker.name || worker.fullName || 'بدون اسم';
  
  // personalImage في DB يحتوي مسار كامل أو نسبي
  // مثل: "/uploads/workers/xxx.jpg" أو "https://..."
  const imageUrl = worker.personalImage || null;
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden active:scale-[0.99] transition">
      <div className="p-4 flex items-start gap-3 cursor-pointer" onClick={onClick}>
        ...
        <div className="flex-shrink-0 relative">
          ...
          <div 
            className="w-14 h-14 rounded-full bg-moh-primary text-white flex items-center justify-center font-bold text-lg"
